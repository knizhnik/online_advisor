...
How it works:
online_advisor use most the same technique as standard Postgres extension auto_explain.
It uses executor hook to enable instrumentation and at the end of query execution
detect quals filtering larger number of records. It is assumed that such quals can be
replaced with index scan.

What it does:
1. Detects plan nodes with number of filtered records exceeds some threshold.
2. Collects column used in this predicate.
3. Propose index creation which involves this columns as keys.
4. Tries to cover different sets of columns with one compound index.

What it doesn't do:
1. Checks operators used in the predicate. For example, query predicate `x > 10 and y < 100` can not be optimized using index on (x,y). But right now online_advisor doesn't detect it.
2. Suggests indexes for table joins or eliminating sort for order by clauses.
3. Estimate effect of present of suggested index. There is another extension - hypothetical indexes https://github.com/HypoPG/hypopg# which allows to do it. It can be used in conjunction with online-advisor.

Requirements:
1. online_advisor should be include in `preload_shared_libraries` list.
...
- "online_advisor.do_instrumentation" - allows to switch on/off instrumentation and so updating of statistic
...
3. By default `online_advisor` tries to propose single compound index covering different use case.
...
which is used by `proposed_indexes` view. You can call it with `combine=false` to report
information separately for each set of variables and prevent online_advisor from
